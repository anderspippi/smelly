...
mkShell rec {
  ...
  shellHook = if stdenv.isDarwin then ''
    export smelly_NO_LTO=
  '' else ''
    export smelly_EGL_LIBRARY='${lib.getLib libGL}/lib/libEGL.so.1'
    export smelly_STARTUP_NOTIFICATION_LIBRARY='${libstartup_notification}/lib/libstartup-notification-1.so'
    export smelly_CANBERRA_LIBRARY='${libcanberra}/lib/libcanberra.so'
  '';
}
