...
smelly has the ability to define its own windows that can be tiled next to each
...
.. figure:: screenshots/screenshot.png
    ...
    running in |smelly| with the *Tall* layout
...
define a few extra key bindings in :file:`smelly.conf`::
...
You can also define shortcuts in :file:`smelly.conf` to make the active window
...
how the available space is split up. To specify the option, in :opt:`smelly.conf
<enabled_layouts>` use::
...
`layout <https://github.com/backbiter-no/smelly/tree/master/smelly/layout>`__
