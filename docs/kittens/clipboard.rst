...
from the shell. It even works over SSH. Using it is as simple as::
    ...
    echo hooray | smelly +kitten clipboard

All text received on :file:`STDIN` is copied to the clipboard.

To get text from the clipboard::

    smelly +kitten clipboard --get-clipboard

The text will be written to :file:`STDOUT`. Note that by default smelly asks for
...
data types. Best illustrated with some examples::
    ...
    smelly +kitten clipboard picture.png

    # Copy an image and some text to the clipboard:
    smelly +kitten clipboard picture.jpg text.txt

    # Copy text from STDIN and an image to the clipboard:
    echo hello | smelly +kitten clipboard picture.png /dev/stdin

    # Copy any raster image available on the clipboard to a PNG file:
    smelly +kitten clipboard -g picture.png

    # Copy an image to a file and text to STDOUT:
    smelly +kitten clipboard -g picture.png /dev/stdout

    # List the formats available on the system clipboard
    smelly +kitten clipboard -g -m . /dev/stdout

Normally, the kitten guesses MIME types based on the file names. To control the
MIME types precisely, use the :option:`--mime <smelly +kitten clipboard --mime>` option.

This kitten uses a new protocol developed by smelly to function, for details,
see :doc:`/clipboard`.

.. program:: smelly +kitten clipboard
