...
|smelly| has a *hints mode* to select and act on arbitrary text snippets
...
create mappings in :file:`smelly.conf` to select various different text
...
the :ref:`smelly config directory <confloc>` with the following contents:
...
Now run smelly with::
    ...
    smelly -o 'map f1 kitten hints --customize-processing custom-hints.py'
...
.. note::
    ...
    :file:`smelly.conf`, creating aliases that have common sets of options.
    For example::
