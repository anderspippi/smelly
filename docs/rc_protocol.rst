The smelly remote control protocol
==================================

The smelly remote control protocol is a simple protocol that involves sending
data to smelly in the form of JSON. Any individual command of smelly has the
form::

    <ESC>P@smelly-cmd<JSON object><ESC>\
...
.. code-block:: json

    {
        ...
        "smelly_window_id": "<Optional value of the smelly_WINDOW_ID env var>",
        "payload": "<Optional JSON object>"
    }

The ``version`` above is an array of the form :code:`[0, 14, 2]`. If you are
developing a standalone client, use the smelly version that you are developing
against. Using a version greater than the version of the smelly instance you are
talking to, will cause a failure.

Set ``no_response`` to ``true`` if you don't want a response from smelly.
...
First, run smelly as::
    ...
    smelly -o allow_remote_control=socket-only --listen-on unix:/tmp/test

Now, in a different terminal, you can get the pretty printed ``@ ls`` output
with the following command line::

    echo -en '\eP@smelly-cmd{"cmd":"ls","version":[0,14,2]}\e\\' | socat - unix:/tmp/test | awk '{ print substr($0, 13, length($0) - 14) }' | jq -c '.data | fromjson' | jq .

There is also the statically compiled stand-alone executable ``kitten``
that can be used for this, available from the `smelly releases
...
the :envvar:`smelly_PUBLIC_KEY` environment variable. Currently, only one
encryption protocol is supported. The protocol number is present in
:envvar:`smelly_PUBLIC_KEY` as ``1``. The key data in this environment variable
