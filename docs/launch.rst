...
|smelly| has a :code:`launch` action that can be used to run arbitrary programs
in new windows/tabs. It can be mapped to user defined shortcuts in
:file:`smelly.conf`. It is very powerful and allows sending the contents of the
current window to the launched program, as well as many other options.

In the simplest form, you can use it to open a new smelly window running the
...
piped has a special environment variable declared, :envvar:`smelly_PIPE_DATA`
whose contents are::
...
``@active-smelly-window-id``
    Replaced by the id of the currently active smelly window.
...
``@input-line-number``
    ...
    scroll position in smelly. See :opt:`scrollback_pager` for details.

``@scrolled-by``
    Replaced by the number of lines smelly is currently scrolled by.
...
For example::
    ...
    map f1 launch my-program @active-smelly-window-id
...
.. code-block:: python
    ...
    from smelly.window import Window
...
in the window and ``window.id`` is the internal smelly ``id`` of the window.
