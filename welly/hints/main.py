...
from smelly.cli_stub import HintsCLIOptions
from smelly.clipboard import set_clipboard_string, set_primary_selection
from smelly.constants import website_url
from smelly.fast_data_types import get_options, wcswidth
from smelly.key_encoding import KeyEvent
from smelly.typing import BossType, smellyCommonOpts
from smelly.utils import ScreenSize, smelly_ansi_sanitizer_pat, resolve_custom_file, screen_size_function
...
def functions_for(args: HintsCLIOptions) -> Tuple[str, List[PostprocessorFunc]]:
    ...
    if args.type == 'url':
        if args.url_prefixes == 'default':
            url_prefixes = smelly_common_opts()['url_prefixes']
        else:
        ...
        if chars is None:
            chars = smelly_common_opts()['select_by_word_characters']
...
def process_escape_codes(text: str) -> Tuple[str, Tuple[Mark, ...]]:
    ...
    text = smelly_ansi_sanitizer_pat().sub(process_hyperlink, text)
    if active_hyperlink_url is not None:
...
:code:`launch`
    run :doc:`/launch` to open the program in a new smelly tab, window, overlay, etc.
    For example::
...
window, :code:`window` a new smelly window, :code:`tab` a new tab,
...
example:
:code:`smelly +kitten hints --type=linenum --linenum-action=tab vim +{line} {path}`
will open the matched path at the matched line number in vim in
a new smelly tab. Note that in order to use :option:`--program` to copy or paste
...
prefixes defined by the :opt:`url_prefixes` option in :file:`smelly.conf`.
...
Defaults to the :opt:`select_by_word_characters` option from :file:`smelly.conf`.
...
Name of a python file in the smelly config directory which will be imported to
...
def parse_hints_args(args: List[str]) -> Tuple[HintsCLIOptions, List[str]]:
    return parse_args(args, OPTIONS, usage, help_text, 'smelly +kitten hints', result_class=HintsCLIOptions)
