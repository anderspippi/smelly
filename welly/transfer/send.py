...
from smelly.fast_data_types import FILE_TRANSFER_CODE, wcswidth
from smelly.file_transmission import Action, Compression, FileTransmissionCommand, FileType, NameReprEnum, TransmissionType, encode_bypass, split_for_transfer
from smelly.typing import KeyEventType, ScreenSize
from smelly.utils import sanitize_control_codes
...
class File:
    def __init__(
        ...
        self.transmit_started_at = self.transmit_ended_at = self.done_at = 0.0
...
class Send(Handler):
    ...
    def __init__(self, cli_opts: TransferCLIOptions, files: Tuple[File, ...]):
        ...
        self.manager = SendManager(random_id(), files, cli_opts.permissions_bypass, cli_opts.transmit_deltas, self.on_file_progress, self.on_file_done)
        ...
        with without_line_wrap(self.write):
            ...
            if p.total_reported_progress > 0:
                self.render_progress(
                    'Total',
                    spinner_char=sc,
                    bytes_so_far=p.total_reported_progress,
    ...
    def draw_progress_for_current_file(self, af: File, spinner_char: str = ' ', is_complete: bool = False) -> None:
        ...
        self.render_progress(
            ...
            spinner_char=spinner_char,
            is_complete=is_complete,
            bytes_so_far=af.reported_progress,
            total_bytes=af.bytes_to_transmit,
