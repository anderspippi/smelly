...
from smelly.cli import CompletionSpec, create_default_opts, parse_args
from smelly.cli_stub import ThemesCLIOptions
from smelly.config import cached_values_for
from smelly.constants import config_dir
from smelly.fast_data_types import truncate_point_for_length, wcswidth
from smelly.options.types import Options as smellyOptions
from smelly.rgb import color_as_sharp, color_from_int
from smelly.typing import KeyEventType
from smelly.utils import ScreenSize
...
class ThemesHandler(Handler):
    ...
    def init_terminal_state(self) -> None:
        ...
        self.cmd.set_window_title('Choose a theme for smelly')
    ...
    def set_colors_to_current_theme(self) -> bool:
        ...
        if self.themes_list:
            o = self.themes_list.current_theme.smelly_opts
        else:
...
By default, this kitten will signal only the parent smelly instance it is
...
instead of changing smelly.conf.


--config-file-name
default=smelly.conf
The name or path to the config file to edit. Relative paths are interpreted
with respect to the smelly config directory. By default the smelly config file,
smelly.conf is edited. This is most useful if you add :code:`include themes.conf`
to your smelly.conf and then have the kitten operate only on :file:`themes.conf`,
allowing :code:`smelly.conf` to remain unchanged.
'''.format


def parse_themes_args(args: List[str]) -> Tuple[ThemesCLIOptions, List[str]]:
    return parse_args(args, OPTIONS, usage, help_text, 'smelly +kitten themes', result_class=ThemesCLIOptions)
