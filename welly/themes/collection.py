...
from smelly.config import atomic_save, parse_config
from smelly.constants import cache_dir, config_dir
from smelly.fast_data_types import Color
from smelly.options.types import Options as smellyOptions
from smelly.utils import reload_conf_in_all_kitties
...
def patch_conf(raw: str, theme_name: str) -> str:
    addition = f'# BEGIN_smelly_THEME\n# {theme_name}\ninclude current-theme.conf\n# END_smelly_THEME'
    nraw, num = re.subn(r'^# BEGIN_smelly_THEME.+?# END_smelly_THEME', addition, raw, flags=re.MULTILINE | re.DOTALL)
    if not num:
        if raw:
            raw += '\n\n'
        nraw = raw + addition
...
def fetch_themes(
    name: str = 'smelly-themes',
    url: str = 'https://codeload.github.com/backbiter-no/smelly-themes/zip/master',
    cache_age: float = 1,
) -> str:
...
def text_as_opts(text: str) -> smellyOptions:
    return smellyOptions(options_dict=parse_config(text.splitlines()))
...
class Theme:
    ...
    def __init__(self, loader: Callable[[], str]):
        ...
        self._opts: Optional[smellyOptions] = None
    ...
    @property
    def smelly_opts(self) -> smellyOptions:
    ...
    def save_in_conf(self, confdir: str, reload_in: str, config_file_name: str = 'smelly.conf') -> None:
        ...
        if reload_in == 'parent':
            if 'smelly_PID' in os.environ:
                os.kill(int(os.environ['smelly_PID']), signal.SIGUSR1)
        elif reload_in == 'all':
