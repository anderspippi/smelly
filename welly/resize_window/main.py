...
from smelly.cli_stub import RCOptions, ResizeCLIOptions
from smelly.constants import version
from smelly.key_encoding import CTRL, EventType, KeyEvent
from smelly.rc.base import command_for_name, parse_subcommand_cli
from smelly.remote_control import encode_send, parse_rc_args
...
class Resize(Handler):
    ...
    def initialize(self) -> None:
        global global_opts
        global_opts = parse_rc_args(['smelly', '@resize-window'])[0]
    ...
    def do_window_resize(self, is_decrease: bool = False, is_horizontal: bool = True, reset: bool = False, multiplier: int = 1) -> None:
        ...
        payload = resize_window.message_to_smelly(global_opts, opts, items)
        send = {'cmd': resize_window.name, 'version': version, 'payload': payload, 'no_response': False}
        self.write(encode_send(send))

    def on_smelly_cmd_response(self, response: Dict[str, Any]) -> None:
