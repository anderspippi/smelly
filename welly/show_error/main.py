...
from smelly.cli_stub import ErrorCLIOptions
from smelly.fast_data_types import open_tty
from smelly.utils import hold_till_enter, no_echo, write_all
...
def real_main(args: List[str]) -> None:
    msg = 'Show an error message. For internal use by smelly.'
    cli_opts, items = parse_args(args[1:], OPTIONS, '', msg, 'smelly +kitten show_error', result_class=ErrorCLIOptions)
    if sys.stdin.isatty():
    ...
    if data.get('tb'):
        ...
        write_all(fd, msg)
        write_all(fd, init_state(alternate_screen=False, smelly_keyboard_mode=False) + set_cursor_visible(False))
        with no_echo(fd):
            termios.tcdrain(fd)
            while True:
