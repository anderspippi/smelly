...
from smelly.cli_stub import BroadcastCLIOptions
from smelly.key_encoding import encode_key_event
from smelly.rc.base import MATCH_TAB_OPTION, MATCH_WINDOW_OPTION
from smelly.remote_control import create_basic_command, encode_send
...
class Broadcast(Handler):
    ...
    def on_key(self, key_event: KeyEventType) -> None:
        ...
        self.write_broadcast_data('smelly-key:' + ek)
...
help_text = 'Broadcast typed text to smelly windows. By default text is sent to all windows, unless one of the matching options is specified'
usage = '[initial text to send ...]'


def parse_broadcast_args(args: List[str]) -> Tuple[BroadcastCLIOptions, List[str]]:
    return parse_args(args, OPTIONS, usage, help_text, 'smelly +kitten broadcast', result_class=BroadcastCLIOptions)
