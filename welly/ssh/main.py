...
from smelly.constants import cache_dir, runtime_dir, shell_integration_dir, ssh_control_master_template, str_version, terminfo_dir
from smelly.shell_integration import as_str_literal
from smelly.shm import SharedMemory
from smelly.types import run_once
from smelly.utils import SSHConnectionData, expandvars, resolve_abs_or_config_path
from smelly.utils import set_echo as turn_off_echo

from ..tui.operations import RESTORE_PRIVATE_MODE_VALUES, SAVE_PRIVATE_MODE_VALUES, Mode, restore_colors, save_colors, set_mode
from ..tui.utils import smelly_opts, running_in_tmux
...
def make_tarfile(ssh_opts: SSHOptions, base_env: Dict[str, str], compression: str = 'gz', literal_env: Dict[str, str] = {}) -> bytes:
    ...
    env = {
        'TERM': os.environ.get('TERM') or smelly_opts().term,
        'COLORTERM': 'truecolor',
    }
    env.update(ssh_opts.env)
    for q in ('smelly_WINDOW_ID', 'WINDOWID'):
        val = os.environ.get(q)
        if val is not None:
            env[q] = val
    env['smelly_SHELL_INTEGRATION'] = ksi or DELETE_ENV_VAR
    env['smelly_SSH_KITTEN_DATA_DIR'] = ssh_opts.remote_dir
    if ssh_opts.login_shell:
        env['smelly_LOGIN_SHELL'] = ssh_opts.login_shell
    if ssh_opts.cwd:
        env['smelly_LOGIN_CWD'] = ssh_opts.cwd
    if ssh_opts.remote_smelly != 'no':
        env['smelly_REMOTE'] = ssh_opts.remote_smelly
    if os.environ.get('smelly_PUBLIC_KEY'):
        env.pop('smelly_PUBLIC_KEY', None)
        literal_env['smelly_PUBLIC_KEY'] = os.environ['smelly_PUBLIC_KEY']
    ...
    with tarfile.open(mode=f'w:{compression}', fileobj=buf, encoding='utf-8') as tf:
        ...
        if ssh_opts.remote_smelly != 'no':
            ...
            tf.add(shell_integration_dir + '/ssh/smelly', arcname=arcname + '/bin/smelly', filter=normalize_tarinfo)
            tf.add(shell_integration_dir + '/ssh/kitten', arcname=arcname + '/bin/kitten', filter=normalize_tarinfo)
        tf.add(f'{terminfo_dir}/smelly.terminfo', arcname='home/.terminfo/smelly.terminfo', filter=normalize_tarinfo)
        tf.add(glob.glob(f'{terminfo_dir}/*/xterm-smelly')[0], arcname='home/.terminfo/x/xterm-smelly', filter=normalize_tarinfo)
    return buf.getvalue()
...
def get_ssh_data(msg: str, request_id: str) -> Iterator[bytes]:
    ...
    try:
        ...
        try:
            ...
            if rq_id != request_id:
                raise ValueError(f'Incorrect request id: {rq_id!r} expecting the smelly_PID-smelly_WINDOW_ID for the current smelly window')
...
def prepare_exec_cmd(remote_args: Sequence[str], is_python: bool) -> str:
    ...
    return f"""unset smelly_SHELL_INTEGRATION; exec "$login_shell" -c '{args}'"""


def prepare_export_home_cmd(ssh_opts: SSHOptions, is_python: bool) -> str:
    home = ssh_opts.env.get('HOME')
    if home == '_smelly_copy_env_var_':
...
def connection_sharing_args(smelly_pid: int) -> List[str]:
    rd = runtime_dir()
...
def dcs_to_smelly(payload: Union[bytes, str], type: str = 'ssh') -> bytes:
    if isinstance(payload, str):
        payload = payload.encode('utf-8')
    payload = standard_b64encode(payload)
    ans = b'\033P@smelly-' + type.encode('ascii') + b'|' + payload
...
def drain_potential_tty_garbage(p: 'subprocess.Popen[bytes]', data_request: str) -> Iterator[None]:
    with open(os.open(os.ctermid(), os.O_CLOEXEC | os.O_RDWR | os.O_NOCTTY), 'wb') as tty:
        if data_request:
            ...
            tty.write(dcs_to_smelly(data_request))
            tty.flush()
        try:
            yield
        finally:
...
def run_ssh(ssh_args: List[str], server_args: List[str], found_extra_args: Tuple[str, ...]) -> NoReturn:
    ...
    if host_opts.share_connections:
        cmd[insertion_point:insertion_point] = connection_sharing_args(int(os.environ['smelly_PID']))
    use_smelly_askpass = host_opts.askpass == 'native' or (host_opts.askpass == 'unless-set' and 'SSH_ASKPASS' not in os.environ)
    need_to_request_data = True
    if use_smelly_askpass:
...
def main(args: List[str]) -> None:
    ...
    if not os.environ.get('smelly_WINDOW_ID') or not os.environ.get('smelly_PID'):
        raise SystemExit('The SSH kitten is meant to run inside a smelly window')
