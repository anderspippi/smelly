...
the remote shell environment. The special value :code:`_smelly_copy_env_var_`
...
the remote host has an architecture for which :link:`pre-compiled smelly binaries
<https://github.com/backbiter-no/smelly/releases>` are available. Note that smelly
is not actually copied to the remote host, instead a small bootstrap script is
copied which will download and run smelly when smelly is first executed on the
remote host. A value of :code:`if-needed` means smelly is installed only if not
already present in the system-wide PATH. A value of :code:`yes` means that smelly
is installed even if already present, and the installed smelly takes precedence.
...
to always use smelly's native, built-in askpass implementation. Note that not
using the smelly askpass implementation means that SSH might need to use the
