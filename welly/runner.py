...
from smelly.constants import list_smelly_resources
from smelly.types import run_once
from smelly.utils import resolve_abs_or_config_path
...
def import_kitten_main_module(config_dir: str, kitten: str) -> Dict[str, Any]:
    ...
    m = importlib.import_module(f'wellies.{kitten}.main')
...
def launch(args: List[str]) -> None:
    ...
    del args[:2]
    args = [kitten] + args
    os.environ['smelly_CONFIG_DIRECTORY'] = config_dir
    set_debug(kitten)
    m = import_kitten_main_module(config_dir, kitten)
    try:
        result = m['start'](args)
...
def run_kitten(kitten: str, run_name: str = '__main__') -> None:
    ...
    if kitten in all_kitten_names():
        runpy.run_module(f'wellies.{kitten}.main', run_name=run_name)
        return
...
def all_kitten_names() -> FrozenSet[str]:
    ans = []
    for name in list_smelly_resources('wellies'):
        if '__' not in name and '.' not in name and name != 'tui':
            ans.append(name)
    return frozenset(ans)


def list_wellies() -> None:
