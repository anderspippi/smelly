...
from smelly.cli_stub import QueryTerminalCLIOptions
from smelly.constants import appname, str_version
from smelly.options.types import Options
from smelly.terminfo import names
...
class Query:
    ...
    def query_name(self) -> str:
        return self.override_query_name or f'smelly-query-{self.name}'
...
class AllowHyperlinks(Query):
    ...
    help_text: str = 'The config option :opt:`allow_hyperlinks` in :file:`smelly.conf` for allowing hyperlinks can be :code:`yes`, :code:`no` or :code:`ask`'
...
class ClipboardControl(Query):
    ...
    help_text: str = 'The config option :opt:`clipboard_control` in :file:`smelly.conf` for allowing reads/writes to/from the clipboard'
...
If a particular :italic:`query` is unsupported by the running smelly version, the
