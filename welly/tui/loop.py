...
from smelly.fast_data_types import FILE_TRANSFER_CODE, close_tty, normal_tty, open_tty, parse_input_from_terminal, raw_tty
from smelly.key_encoding import ALT, CTRL, SHIFT, backspace_key, decode_key_event, enter_key
from smelly.typing import ImageManagerType, KeyEventType, Protocol
from smelly.utils import ScreenSize, ScreenSizeGetter, screen_size_function, write_all
...
def debug_write(*a: Any, **kw: Any) -> None:
    ...
    for i in range(0, len(stext), 256):
        chunk = stext[i:i + 256]
        text = b'\x1bP@smelly-print|' + standard_b64encode(chunk.encode('utf-8')) + b'\x1b\\'
        fobj.write(text)
    fobj.flush()
...
class Loop:
    ...
    def _on_dcs(self, dcs: str) -> None:
        if dcs.startswith('@smelly-cmd'):
            import json
            self.handler.on_smelly_cmd_response(json.loads(dcs[len('@smelly-cmd'):]))
    ...
    def _on_osc(self, osc: str) -> None:
        ...
        if q == '52':
            ...
            from smelly.file_transmission import FileTransmissionCommand
            data = memoryview(osc.encode('ascii'))
