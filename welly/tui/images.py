...
from smelly.conf.utils import positive_float, positive_int
from smelly.fast_data_types import create_canvas
from smelly.typing import GRT_C, CompletedProcess, GRT_a, GRT_d, GRT_f, GRT_m, GRT_o, GRT_t, HandlerType
from smelly.utils import ScreenSize, fit_image, which
    ...
    def check_resize(frame: Frame) -> None:
        ...
        if sz < expected_size:
            ...
            if missing % (bytes_per_pixel * width) != 0:
                raise ConvertFailed(
                    path,
                    'ImageMagick failed to convert {} correctly,'
                    ' it generated {} < {} of data (w={}, h={}, bpp={})'.format(path, sz, expected_size, frame.width, frame.height, bytes_per_pixel),
                )
