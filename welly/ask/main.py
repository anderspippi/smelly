...
from smelly.cli_stub import AskCLIOptions
...
from smelly.typing import BossType, KeyEventType, TypedDict
...
def main(args: List[str]) -> Response:
    # For some reason importing readline in a key handler in the main smelly process
    # causes a crash of the python interpreter, probably because of some global
    # lock
    global readline
    msg = 'Ask the user for input'
    try:
        cli_opts, items = parse_args(args[1:], option_text, '', msg, 'smelly +kitten ask', result_class=AskCLIOptions)
