...
from smelly.cli_stub import RemoteFileCLIOptions
from smelly.constants import cache_dir
from smelly.typing import BossType
from smelly.utils import SSHConnectionData, command_for_open, get_editor, open_cmd
    ...
    def check_hostname_matches(self) -> bool:
        ...
        if cp.returncode == 0:
            ...
            if not hostname_matches(self.cli_opts.hostname or '', q):
                ...
                print('This indicates that smelly has not connected to the correct remote machine.')
                print('This can happen, for example, when using nested SSH sessions.')
                print(f'The hostname smelly used to connect was: {styled(self.conn_data.hostname, fg="yellow")}', end='')
...
def main(args: List[str]) -> Result:
    msg = 'Ask the user what to do with the remote file. For internal use by smelly, do not run it directly.'
    try:
        cli_opts, items = parse_args(args[1:], option_text, '', msg, 'smelly +kitten remote_file', result_class=RemoteFileCLIOptions)
