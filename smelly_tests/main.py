...
def type_check() -> NoReturn:
    from smelly.cli_stub import generate_stub  # type:ignore

    generate_stub()
    from wellies.tui.operations_stub import generate_stub  # type: ignore
...
def run_go(packages: Set[str], names: str) -> 'subprocess.Popen[bytes]':
    go = go_exe()
    go_pkg_args = [f'smelly/{x}' for x in packages]
    cmd = [go, 'test', '-v']
    for name in names:
        cmd.extend(('-run', name))
    cmd += go_pkg_args
