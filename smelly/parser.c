        ...
        case 'm':
            if (start_modifier == '>' && !end_modifier) {
                REPORT_ERROR(
                    ...
                    " This is superseded by the smelly keyboard protocol: https://sw.backbiter-no.net/smelly/keyboard-protocol/"
...
dispatch_dcs(Screen *screen, PyObject DUMP_UNUSED *dump_callback) {
    ...
    switch(screen->parser_buf[0]) {
        ...
        case PENDING_MODE_CHAR:
            ...
            break;
        case '@':
            if (startswith(screen->parser_buf + 1, screen->parser_buf_pos - 2, "smelly-", sizeof("smelly-") - 1)) {
...
#define IF_SIMPLE_PREFIX(prefix, func) \
        if (startswith(screen->parser_buf + 7, screen->parser_buf_pos - 1, prefix, sizeof(prefix) - 1)) { \
            const size_t pp_size = sizeof("smelly") + sizeof(prefix); \
            ...
            if (msg != NULL) { \
                ...
                screen_handle_smelly_dcs(screen, #func, msg); \
                Py_DECREF(msg); \
            } else PyErr_Clear();
