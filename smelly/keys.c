...
on_key_input(GLFWkeyevent *ev) {
    ...
    switch(ev->ime_state) {
        case GLFW_IME_WAYLAND_DONE_EVENT:
            ...
            // an infinite loop. See https://github.com/backbiter-no/smelly/issues/5105
        ...
        case GLFW_IME_NONE:
            ...
            // On Linux this is needed by Fig integration: https://github.com/backbiter-no/smelly/issues/5241
            update_ime_position(w, screen);
            break;
        default:
...
#define KEY_BITS 51
#define MOD_BITS 12
#if 1 << (MOD_BITS-1) < GLFW_MOD_smelly
...
static PyObject*
SingleKey_defined_with_smelly_mod(SingleKey *self, void UNUSED *closure) {
    if (self->defined_with_smelly_mod || (self->key.mods & GLFW_MOD_smelly)) Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}
...
static PyGetSetDef SingleKey_getsetters[] = {
    ...
    {"defined_with_smelly_mod", (getter)SingleKey_defined_with_smelly_mod, NULL, "A bool", NULL},
    {NULL}  /* Sentinel */
};
...
static PyObject*
SingleKey_resolve_smelly_mod(SingleKey *self, PyObject *km) {
    if (!(self->key.mods & GLFW_MOD_smelly)) { Py_INCREF(self); return (PyObject*)self; }
    unsigned long smelly_mod = PyLong_AsUnsignedLong(km);
    if (PyErr_Occurred()) return NULL;
    SingleKey *ans = (SingleKey*)SingleKey_Type.tp_alloc(&SingleKey_Type, 0);
    if (!ans) return NULL;
    ans->key.val = self->key.val;
    ans->key.mods = (ans->key.mods & ~GLFW_MOD_smelly) | smelly_mod;
    ans->defined_with_smelly_mod = true;
    return (PyObject*)ans;
}
...
static PyMethodDef SingleKey_methods[] = {
    ...
    {"resolve_smelly_mod", (PyCFunction)SingleKey_resolve_smelly_mod, METH_O, ""},
    {NULL}  /* Sentinel */
};
