...
website_base_url = 'https://sw.backbiter-no.net/smelly/'
default_pager_for_help = ('less', '-iRXF')
if getattr(sys, 'frozen', False):
    extensions_dir: str = getattr(sys, 'smelly_run_data')['extensions_dir']
    ...
    if not rpath:
        ...
        for candidate in filter(None, items):
            if candidate not in seen:
                seen.add(candidate)
                if os.access(os.path.join(candidate, 'smelly'), os.X_OK):
                    rpath = candidate
                    break
        else:
            raise RuntimeError('smelly binary not found')
    return os.path.join(rpath, 'smelly')


@run_once
def kitten_exe() -> str:
    return os.path.join(os.path.dirname(smelly_exe()), 'kitten')
...
del _get_config_dir
defconf = os.path.join(config_dir, 'smelly.conf')
...
@run_once
def runtime_dir() -> str:
    if 'smelly_RUNTIME_DIRECTORY' in os.environ:
...
ssh_control_master_template = 'kssh-{smelly_pid}-{ssh_placeholder}'


def glfw_path(module: str) -> str:
    prefix = 'smelly.' if getattr(sys, 'frozen', False) else ''
...
def detect_if_wayland_ok() -> bool:
    if 'WAYLAND_DISPLAY' not in os.environ:
        return False
    if 'smelly_DISABLE_WAYLAND' in os.environ:
        return False
...
def running_in_smelly(set_val: Optional[bool] = None) -> bool:
    if set_val is not None:
        setattr(running_in_smelly, 'ans', set_val)
    return bool(getattr(running_in_smelly, 'ans', False))
...
def local_docs() -> str:
    d = os.path.dirname
    base = d(d(smelly_exe()))
    from_source = getattr(sys, 'smelly_run_data').get('from_source')
    if is_macos and from_source and '/smelly.app/Contents/' in smelly_exe():
        base = d(d(d(base)))
    subdir = os.path.join('doc', 'smelly', 'html')
