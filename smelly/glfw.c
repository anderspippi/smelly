/*
 * Copyright (C) 2016 anders Goyal <anders at backbiter-no.net>
 *
...
create_os_window(PyObject UNUSED *self, PyObject *args, PyObject *kw) {
    ...
    if (!apple_preserve_common_context) {
        apple_preserve_common_context = glfwCreateWindow(640, 480, "smelly", NULL, common_context);
    }
    ...
    if (!global_state.is_wayland) {
        ...
        if (temp_window == NULL) { fatal("Failed to create GLFW temp window! This usually happens because of old/broken OpenGL drivers. smelly requires working OpenGL 3.3 drivers."); }
    }
    float xscale, yscale;
    double xdpi, ydpi;
...
#else
    if (OPT(bell_path)) play_canberra_sound(OPT(bell_path), "smelly bell", true, "event");
    else play_canberra_sound("bell", "smelly bell", false, "event");
#endif
}
...
void
request_window_attention(id_type smelly_window_id, bool audio_bell) {
    OSWindow *w = os_window_for_smelly_window(smelly_window_id);
    if (w) {
...
request_frame_render(OSWindow *w) {
    ...
    // render frame requests, see https://github.com/backbiter-no/smelly/issues/2329
