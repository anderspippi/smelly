...
#define SIDE_EFFECTS(name) case name: if (do_side_effects) set_mode_from_const(self, name, src->m##name); else dest->m##name = src->m##name; break;
    ...
    switch(mode) {
        SIMPLE_MODE(LNM)  // smelly extension
        SIMPLE_MODE(IRM)  // smelly extension
        SIMPLE_MODE(DECARM)
        SIMPLE_MODE(BRACKETED_PASTE)
        SIMPLE_MODE(FOCUS_TRACKING)
        SIMPLE_MODE(DECCKM)
...
screen_save_modes(Screen *self) {
    // smelly extension to XTSAVE that saves a bunch of no side-effect modes
...
screen_restore_modes(Screen *self) {
    // smelly extension to XTRESTORE that saves a bunch of no side-effect modes
...
screen_xtversion(Screen *self, unsigned int mode) {
    if (mode == 0) {
        write_escape_code_to_child(self, DCS, ">|smelly(" XT_VERSION ")");
    }
}
...
void screen_handle_smelly_dcs(Screen *self, const char *callback_name, PyObject *cmd) {
