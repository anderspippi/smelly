...
def runpy(args: List[str]) -> None:
    if len(args) < 2:
        raise SystemExit('Usage: smelly +runpy "some python code"')
    sys.argv = ['smelly'] + args[2:]
    exec(args[1])
...
def launch(args: List[str]) -> None:
    ...
    try:
        ...
        raise SystemExit(
            'usage: smelly +launch script.py [arguments to be passed to script.py ...]\n\n'
            'script.py will be run with full access to smelly code. If script.py is '
...
def namespaced(args: List[str]) -> None:
    try:
        ...
        raise SystemExit('The smelly command line is incomplete')
    except KeyError:
        pass
    else:
        func(args[1:])
...
def setup_openssl_environment(ext_dir: str) -> None:
    ...
    setattr(sys, 'smelly_ssl_env_var', 'SSL_CERT_FILE')


def main() -> None:
    if getattr(sys, 'frozen', False):
        ext_dir: str = getattr(sys, 'smelly_run_data').get('extensions_dir')
        if ext_dir:
