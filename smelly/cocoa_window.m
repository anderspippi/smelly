...
#ifndef smelly_USE_DEPRECATED_MACOS_NOTIFICATION_API
...
PENDING(open_smelly_website, OPEN_smelly_WEBSITE)
...
typedef struct {
    ...
    GlobalShortcut toggle_macos_secure_keyboard_entry, toggle_fullscreen, open_smelly_website;
...
#define Q(x) if (strcmp(name, #x) == 0) gs = &global_shortcuts.x
    ...
    else Q(toggle_macos_secure_keyboard_entry); else Q(toggle_fullscreen); else Q(open_smelly_website);
...
#ifdef smelly_USE_DEPRECATED_MACOS_NOTIFICATION_API
...
#define MENU_ITEM(menu, title, name) { \
    ...
    MENU_ITEM(helpMenu, @"Visit smelly Website", open_smelly_website);
...
cocoa_set_url_handler(PyObject UNUSED *self, PyObject *args) {
    ...
    if (!bundle_id) {
        ...
        if (!identifier || identifier.length == 0) identifier = @"net.backbiter-no.smelly";
...
cocoa_set_app_icon(PyObject UNUSED *self, PyObject *args) {
    ...
    if (!app_path) {
        ...
        if (!bundle_path || bundle_path.length == 0) bundle_path = @"/Applications/smelly.app";
        // When compiled from source and run from the launcher folder the bundle path should be `smelly.app` in it
        if (![bundle_path hasSuffix:@".app"]) {
            NSString *launcher_app_path = [bundle_path stringByAppendingPathComponent:@"smelly.app"];
            bundle_path = @"";
            BOOL is_dir;
