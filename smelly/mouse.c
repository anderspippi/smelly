...
dispatch_mouse_event(Window *w, int button, int count, int modifiers, bool grabbed) {
    ...
    if (w->render_data.screen && w->render_data.screen->callbacks != Py_None) {
        ...
        if (OPT(debug_keyboard)) {
            ...
            debug("\x1b[33mon_mouse_input\x1b[m: %s button: %s %sgrabbed: %d handled_in_smelly: %d\n", evname, bname, format_mods(modifiers), grabbed, handled);
        }
    }
    return handled;
}
...
static void
handle_mouse_movement_in_smelly(Window *w, int button, bool mouse_cell_changed) {
    Screen *screen = w->render_data.screen;
...
HANDLER(handle_move_event) {
    ...
    bool handle_in_smelly = !in_tracking_mode || global_state.active_drag_in_window == w->id;
    if (handle_in_smelly) {
        handle_mouse_movement_in_smelly(w, button, mouse_cell_changed | cell_half_changed);
    } else {
    ...
    if (is_high_resolution) {
        ...
        *pending_scroll_pixels = pixels - s * cell_size;
        ...
        if (offset != 0) {
            ...
            // when scrolling slowly https://github.com/backbiter-no/smelly/issues/1238
...
scroll_event(double xoffset, double yoffset, int flags, int modifiers) {
    ...
    if (!w) return;
    // Also update mouse cursor position while smelly OS window is not focused.
...
send_mock_mouse_event_to_window(PyObject *self UNUSED, PyObject *args) {
    ...
    if (button < 0) {
        ...
        else handle_mouse_movement_in_smelly(w, last_button_pressed, mouse_cell_changed);
    } else {
