...
def set_macos_app_custom_icon() -> None:
    for name in ('smelly.app.icns', 'smelly.app.png'):
        ...
        if custom_icon_mtime is not None:
            ...
            # the application icon here. This will revert when smelly quits, but
...
def setup_profiling() -> Generator[None, None, None]:
    try:
        ...
        do_profile = False
    if do_profile:
        start_profiler('/tmp/smelly-profile.log')
    yield
    if do_profile:
        import subprocess
...
def expand_listen_on(listen_on: str, from_config_file: bool) -> str:
    ...
    if '{smelly_pid}' not in listen_on and from_config_file:
...
def ensure_smelly_in_path() -> None:
    # Ensure the correct smelly is in PATH
    krd = getattr(sys, 'smelly_run_data')
    ...
    if rpath:
        ...
        if modify_path or not existing:
            ...
            correct_smelly = os.path.join(rpath, 'smelly')
            if not existing or not safe_samefile(existing, correct_smelly):
...
def setup_manpath(env: Dict[str, str]) -> None:
    # Ensure smelly manpages are available in frozen builds
    ...
    if not mp:
        env['MANPATH'] = f'{smelly_man}:'
    elif mp.startswith(':'):
        env['MANPATH'] = f':{smelly_man}:{mp}'
    else:
        env['MANPATH'] = f'{smelly_man}:{mp}'
...
def setup_environment(opts: Options, cli_opts: CLIOptions) -> None:
    ...
    smelly_path = shutil.which('smelly')
    if smelly_path:
...
def _main() -> None:
    ...
    if getattr(sys, 'cmdline_args_for_open', False):
        ...
        appname: Optional[str] = 'smelly +open'
        msg: Optional[str] = (
