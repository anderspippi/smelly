...
#define smelly_HANDLED_SIGNALS SIGINT, SIGHUP, SIGTERM, SIGCHLD, SIGUSR1, SIGUSR2, 0
...
static PyObject*
mask_smelly_signals_process_wide(PyObject *self UNUSED, PyObject *a UNUSED) {
    mask_variadic_signals(0, smelly_HANDLED_SIGNALS);
    Py_RETURN_NONE;
}
...
new(PyTypeObject *type, PyObject *args, PyObject UNUSED *kwds) {
    ...
    if (!init_loop_data(&self->io_loop_data, smelly_HANDLED_SIGNALS)) return PyErr_SetFromErrno(PyExc_OSError);
...
thread_write(void *x) {
    ...
    set_thread_name("smellyWriteStdin");
...
process_cocoa_pending_actions(void) {
    ...
    if (cocoa_pending_actions[OPEN_smelly_WEBSITE]) { call_boss(open_smelly_website, NULL); }
...
#ifdef smelly_PRINT_BYTES_SENT_TO_CHILD
static void
...
io_loop(void *data) {
    ...
    set_thread_name("smellyChildMon");
...
#define smelly_CMD_PREFIX "\x1bP@smelly-cmd{"
...
has_complete_peer_command(Peer *peer) {
    ...
    if (peer->read.used > sizeof(smelly_CMD_PREFIX) && memcmp(peer->read.data, smelly_CMD_PREFIX, sizeof(smelly_CMD_PREFIX)-1) == 0) {
        for (size_t i = sizeof(smelly_CMD_PREFIX)-1; i < peer->read.used - 1; i++) {
...
static PyMethodDef module_methods[] = {
    ...
    METHODB(mask_smelly_signals_process_wide, METH_NOARGS),
