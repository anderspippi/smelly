...
#ifndef smelly_LIB_PATH
#define smelly_LIB_PATH "../.."
#endif
#ifndef smelly_LIB_DIR_NAME
#define smelly_LIB_DIR_NAME "lib"
#endif
...
static bool
set_smelly_run_data(RunData *run_data, bool from_source, wchar_t *extensions_dir) {
...
#undef S
    int ret = PySys_SetObject("smelly_run_data", ans);
    Py_CLEAR(ans);
...
run_embedded(RunData *run_data) {
...
#else
    const char *python_relpath = "../" smelly_LIB_DIR_NAME;
#endif
    int num = snprintf(extensions_dir_full, PATH_MAX, "%s/%s/smelly-extensions", run_data->exe_dir, python_relpath);
    ...
    bypy_initialize_interpreter(
            L"smelly", python_home, L"smelly_main", extensions_dir, run_data->argc, run_data->argv);
    if (!set_smelly_run_data(run_data, false, extensions_dir)) return 1;
...
run_embedded(RunData *run_data) {
    ...
    if (!set_smelly_run_data(run_data, from_source, NULL)) return 1;
...
read_exe_path(char *exe, size_t buf_sz) {
    ...
    while (token != NULL) {
        char q[PATH_MAX + 1] = {0};
        snprintf(q, PATH_MAX, "%s/smelly", token);
        if (safe_realpath(q, exe, buf_sz)) return true;
        token = strtok(NULL, ":");
    }
    fprintf(stderr, "smelly not found in PATH aborting\n");
    return false;
}
...
is_wrapped_kitten(const char *arg) {
    ...
    return strstr(" " WRAPPED_wellies " ", buf);
}
...
int main(int argc, char *argv[], char* envp[]) {
    ...
    num = snprintf(lib, PATH_MAX, "%s/%s", exe_dir, smelly_LIB_PATH);

    if (num < 0 || num >= PATH_MAX) { fprintf(stderr, "Failed to create path to smelly lib\n"); return 1; }
