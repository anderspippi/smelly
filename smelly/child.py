...
class Child:
    ...
    def final_env(self) -> Dict[str, str]:
        ...
        env['smelly_PID'] = getpid()
        env['smelly_PUBLIC_KEY'] = boss.encryption_public_key
        if self.add_listen_on_env_var and boss.listening_on:
            env['smelly_LISTEN_ON'] = boss.listening_on
        else:
            env.pop('smelly_LISTEN_ON', None)
        if self.cwd:
        ...
        if tdir:
            env['TERMINFO'] = tdir
        env['smelly_INSTALLATION_DIR'] = smelly_base_dir
        ...
        if self.is_clone_launch:
            env['smelly_IS_CLONE_LAUNCH'] = self.is_clone_launch
    ...
    def fork(self) -> Optional[int]:
        ...
        if is_macos and exe == shell_path:
            ...
            # https://github.com/backbiter-no/smelly/issues/247
