...
from smelly.conf.types import Definition, MultiOption, Option, unset
from smelly.types import _T
...
def generate_class(defn: Definition, loc: str) -> Tuple[str, str]:
    ...
    ki_imports: 're.Pattern[str]' = re.compile(r'\b((?:wellies|smelly).+?)[,\]]')
    ...
    for option in sorted(defn.iter_all_options(), key=lambda a: natural_keys(a.name)):
        ...
        if option.macos_defval is not unset:
            ...
            imports.add(('smelly.constants', 'is_macos'))
    ...
    t('    return ans')
    tc_imports.add(('smelly.conf.utils', 'merge_dicts'))
...
def main() -> None:
    # To use run it as:
    # smelly +runpy 'from smelly.conf.generate import main; main()' /path/to/kitten/file.py
    import importlib
    import sys

    from wellies.runner import path_to_custom_kitten, resolved_kitten
