...
import smelly.conf.utils as generic_parsers
...
def resolve_ref(ref: str, website_url: Callable[[str], str] = website_url) -> str:
    ...
    if href:
        pass
    elif ref.startswith('conf-smelly-'):
        href = f'conf#{ref}'
    elif ref.startswith('conf-kitten-'):
        parts = ref.split('-')
        href = "wellies/" + parts[2] + f'/#{ref}'
        ...
        href = f'https://github.com/backbiter-no/smelly/{prefix}/{rest}'
...
class CoalescedIteratorData:
    ...
    def initialize(self, root: 'Group') -> None:
        ...
        self.smelly_mod = 'smelly_mod'
        for item in root.iter_all_non_groups():
            if isinstance(item, Option):
                if item.name == 'smelly_mod':
                    self.smelly_mod = item.defval_as_string
                if current_group:
...
class MultiOption:
    ...
    def as_rst(self, conf_name: str, shortcut_slugs: Dict[str, Tuple[str, str]], smelly_mod: str, level: int = 0) -> List[str]:
        ans: List[str] = []
        a = ans.append
...
class Mapping:
    ...
    def as_rst(
        ...
        shortcut_slugs[f'{conf_name}.{self.name}'] = (sc_text, self.key_text.replace('smelly_mod', smelly_mod))
        ...
        for sc in [self] + action_group:
            if sc.add_to_default and sc.documented:
                ...
                if sc.only == 'macos':
                    ...
                    suffix = ' 🐧'
                a(f'    {sc.setting_name} {sc.parseable_text.replace("smelly_mod", smelly_mod)}{suffix}')
        a('')
        if self.long_text:
            a('')
            a(expand_opt_references(conf_name, self.long_text))
...
class Group:
    ...
    def as_rst(self, conf_name: str, shortcut_slugs: Dict[str, Tuple[str, str]], smelly_mod: str = 'smelly_mod', level: int = 0) -> List[str]:
        ans: List[str] = []
        a = ans.append
        if level:
            a('')
        ...
        smelly_mod = self.coalesced_iterator_data.smelly_mod
        for item in self.iter_with_coalesced_options():
            if isinstance(item, Option):
                lines = item.as_rst(conf_name, shortcut_slugs, smelly_mod, option_group=self.coalesced_iterator_data.option_group_for_option(item))
            elif isinstance(item, Mapping):
                lines = item.as_rst(conf_name, shortcut_slugs, smelly_mod, level + 1, action_group=self.coalesced_iterator_data.action_group_for_action(item))
            else:
                lines = item.as_rst(conf_name, shortcut_slugs, smelly_mod, level + 1)
            ans.extend(lines)
