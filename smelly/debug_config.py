...
from wellies.tui.operations import colored, styled

from .cli import version
from .constants import extensions_dir, is_macos, is_wayland, smelly_base_dir, smelly_exe, shell_path
...
def debug_config(opts: smellyOpts) -> str:
    ...
    p(green('Paths:'))
    p(yellow('  smelly:'), os.path.realpath(smelly_exe()))
    p(yellow('  base dir:'), smelly_base_dir)
    ...
    compare_opts(opts, p)
    p()
    p(green('Important environment variables seen by the smelly process:'))
    ...
    for k in (
        'PATH LANG smelly_CONFIG_DIRECTORY smelly_CACHE_DIRECTORY VISUAL EDITOR SHELL'
        ' GLFW_IM_MODULE smelly_WAYLAND_DETECT_MODIFIERS DISPLAY WAYLAND_DISPLAY USER XCURSOR_SIZE'
    ).split():
        penv(k)
    for k in os.environ:
