...
def handle_cmd(
    ...
    if tuple(v)[:2] > version[:2]:
        if no_response:
            return None
        return {'ok': False, 'error': 'The smelly client you are using to send remote commands is newer than this smelly instance. This is not supported.'}
...
--to
An address for the smelly instance to control. Corresponds to the address given
to the smelly instance via the :option:`smelly --listen-on` option or the
:opt:`listen_on` setting in :file:`smelly.conf`. If not specified, the
environment variable :envvar:`smelly_LISTEN_ON` is checked. If that is also not
found, messages are sent to the controlling terminal for this process, i.e.
they will only work if this process is run within a smelly window.


--password
A password to use when contacting smelly. This will cause smelly to ask the user
for permission to perform the specified action, unless the password has been
accepted before or is pre-configured in :file:`smelly.conf`.
...
paths are resolved from the smelly configuration directory. Use - to read from STDIN.
Used if no :option:`smelly @ --password` is supplied. Defaults to checking for the
:file:`rc-pass` file in the smelly configuration directory.


--password-env
default=smelly_RC_PASSWORD
The name of an environment variable to read the password from.
Used if no :option:`smelly @ --password-file` is supplied. Defaults
to checking the environment variable :envvar:`smelly_RC_PASSWORD`.


--use-password
default=if-available
choices=if-available,never,always
If no password is available, smelly will usually just send the remote control command
