...
from smelly.constants import smelly_exe, running_in_smelly
from smelly.entry_points import main as main_entry_point
...
from smelly.types import SignalInfo
from smelly.utils import log_error, safer_fork
...
def exec_main(stdin_read: int, stdout_write: int, death_notify_write: int) -> None:
    ...
    running_in_smelly(False)
    for x in (sys.stdout, sys.stdin, sys.stderr):
        if not x.line_buffering:  # happens if the parent smelly instance has stdout not pointing to a terminal
