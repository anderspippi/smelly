...
# in smelly, when using the spawn launcher.
...
def spawnv_passfds(path: bytes, args: ArgsType, passfds: Sequence[int]) -> int:
    if '-c' in args:
        idx = args.index('-c')
        patched_args = [spawn.get_executable(),
                        '+runpy'] + list(args)[idx + 1:]
    else:
...
def get_process_pool_executor(
        ...
        initargs: Tuple[Any, ...] = ()) -> ProcessPoolExecutor:
    if prefer_fork and 'fork' in get_all_start_methods():
        ctx: Union[context.DefaultContext,
                   context.ForkContext] = get_context('fork')
    else:
