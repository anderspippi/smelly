...
def setup_fish_env(env: Dict[str, str], argv: List[str]) -> None:
    ...
    env['smelly_FISH_XDG_DATA_DIR'] = shell_integration_dir
    if not val:
...
def get_zsh_zdotdir_from_global_zshenv(
        env: Dict[str, str],
        argv: List[str]) -> Optional[str]:
    exe = which(argv[0], only_system=True) or 'zsh'
    with suppress(Exception):
        return subprocess.check_output(
            [exe, '--norcs', '--interactive', '-c', 'echo -n $ZDOTDIR'],
...
def setup_zsh_env(env: Dict[str, str], argv: List[str]) -> None:
    ...
    if zdotdir is not None:
        env['smelly_ORIG_ZDOTDIR'] = zdotdir
    else:
        # smelly_ORIG_ZDOTDIR can be set at this point if, for example, the global
        # zshenv overrides ZDOTDIR; we try to limit the damage in this case
        env.pop('smelly_ORIG_ZDOTDIR', None)
...
def setup_bash_env(env: Dict[str, str], argv: List[str]) -> None:
    ...
    if file_arg_set and not interactive_opt:
        # non-interactive shell
        return
    env['ENV'] = os.path.join(shell_integration_dir, 'bash', 'smelly.bash')
    env['smelly_BASH_INJECT'] = ' '.join(inject)
    if posix_env:
        env['smelly_BASH_POSIX_ENV'] = posix_env
    if rcfile:
        env['smelly_BASH_RCFILE'] = rcfile
...
def posix_serialize_env(
        env: Dict[str, str],
        prefix: str = 'builtin export', sep: str = '=') -> str:
    ans = []
