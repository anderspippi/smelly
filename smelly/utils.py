...
@run_once
def smelly_ansi_sanitizer_pat() -> 're.Pattern[str]':
    # removes ANSI sequences generated by smelly's ANSI output routines. Not
...
def load_shaders(name: str, vertex_name: str = '', fragment_name: str = '') -> Tuple[str, str]:
    ...
    def load(which: str, lname: str = '') -> str:
        lname = lname or name
        return read_smelly_resource(f'{lname}_{which}.glsl').decode('utf-8').replace('GLSL_VERSION', str(GLSL_VERSION), 1)
...
def is_smelly_gui_cmdline(*cmd: str) -> bool:
    if not cmd:
        return False
    if os.path.basename(cmd[0]) != 'smelly':
        return False
    if len(cmd) == 1:
        return True
    s = cmd[1][:1]
...
def reload_conf_in_all_kitties() -> None:
    ...
    for pid in get_all_processes():
        try:
            ...
            continue
        if cmd and is_smelly_gui_cmdline(*cmd):
...
def path_from_osc7_url(url: str) -> str:
    if url.startswith('smelly-shell-cwd://'):
