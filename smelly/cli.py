...
from .options.types import Options as smellyOpts
...
If the environment variable :envvar:`smelly_CONFIG_DIRECTORY` is specified, that
...
def ref_hyperlink(x: str, prefix: str = '') -> str:
    t, q = text_and_target(x)
    url = f'smelly+doc://{hostname()}/#ref={prefix}{q}'
...
inside smelly, permanently fixing the OS window's title. So only use this if you
...
--override -o
type=list
completion=type:special group:complete_smelly_override
...
relative paths are resolved relative to the smelly configuration directory.
...
--listen-on
completion=type:special group:complete_smelly_listen_on
Listen on the specified socket address for control messages. For example,
:option:`{appname} --listen-on`=unix:/tmp/mysmelly or :option:`{appname}
...
--listen-on`=unix:@mysmelly. Environment variables are expanded and relative
paths are resolved with respect to the temporary directory. To control smelly,
...
:italic:`{appname} @` within a smelly window, there is no need to specify the
...
Control how the initial smelly window is created.
...
can open a new smelly window to replay the commands with::
