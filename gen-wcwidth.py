...
def gen_emoji() -> None:
    with create_header('smelly/emoji.h') as p:
...
def gen_ucd() -> None:
    ...
    with create_header('smelly/unicode-data.c') as p:
        ...
        p('}\n')
        with open('smelly/unicode-data.h', 'r+') as f:
            raw = f.read()
            f.seek(0)
            raw, num = re.subn(
...
def gen_wcwidth() -> None:
    ...
    with create_header('smelly/wcwidth-std.h') as p, open('tools/wcswidth/std.go', 'w') as gof:
